...
export const ProductForm = () => {
  ...
  // Default a new product to the first available category once it loads.
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        await api.putForm(`/api/products/${id}`, formData);
      } else {
        await api.postForm('/api/products', formData);
      }
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <Link to="/admin/products" className="inline-flex items-center gap-1.5 text-sm font-semibold text-brand-600 dark:text-brand-300">
